...
from .pct_tokenizer_heatmap import PCT_Heatmap_Tokenizer
from .modules import MixerLayer, FCBlock, BasicBlock
from evaluate.eval import pose_pck_accuracy


@HEADS.register_module()
class PCT_Heatmap_Head(TopdownHeatmapBaseHead):
    ...
    def __init__(self,
        ...
        self.tokenizer = PCT_Heatmap_Tokenizer(
            stage_pct=stage_pct, tokenizer=tokenizer, num_joints=num_joints, heatmap_size=heatmap_size)
    ...
    def forward(self, x, target=None, target_weight=None, train=True):
        """Forward function."""
        ...
        recoverd_heatmap, cls_label, e_latent_loss = \
            self.tokenizer(target, target_weight, cls_logits_softmax, train=train)

        if train:
            return cls_logits, recoverd_heatmap, cls_label, e_latent_loss
        else:
            return recoverd_heatmap, encoding_scores
