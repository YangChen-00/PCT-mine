...
class PCT_VAE(BasePose):
    def __init__(self,
        ...
        assert self.stage_pct in ["tokenizer", "predictor"]
        ...
        if self.stage_pct == "predictor":
            ...
            # backbone is only needed for training predictor
    ...
    def forward(self,
        ...
        if joints_3d is not None and joints_3d_visible is not None:
            joints = joints_3d
    ...
    def forward_train(self, img, joints, img_metas, **kwargs):
        ...
        """
            output_joints - 分类网络输出的logits（only Stage2）
            mu - Decoder得出的关节点坐标（Stage1 and Stage2）
            log_var - Encoder得出的最相似token的index（Stage1 and Stage2）
            dist_z  - 计算离最相似的token的相似距离作为e_latent_loss（only Stage1）
        """
        ...
        if self.stage_pct == "predictor":
            keypoint_losses = self.keypoint_head.get_loss(
                output_joints, joints, mu, log_var)
    ...
    def forward_test(self, img, joints, img_metas, **kwargs):
        ...
        num_joints = output_joints.shape[1]
        score_pose = joints[:,:,2:] if joints is not None else np.ones((batch_size, num_joints, 1))
        ...
        if self.flip_test:
            ...
            output_joints_f, mu, log_var, dist_z = \
                self.keypoint_head(features_flipped, \
                    extra_output_flipped, joints_flipped, train=False)

            output_joints_f = output_joints_f[:,FLIP_INDEX[self.dataset_name],:]
            output_joints_f[:,:,0] = img.shape[-1] - 1 - output_joints_f[:,:,0]
            
            score_pose_f = joints[:,:,2:] if joints is not None else np.ones((batch_size, num_joints, 1))

            output_joints = (output_joints + output_joints_f)/2.0
        ...
        output_joints = output_joints.cpu().numpy()
        if isinstance(score_pose, torch.Tensor):
            score_pose = score_pose.cpu().numpy()
        for i in range(output_joints.shape[0]):
            output_joints[i] = transform_preds(
                output_joints[i], c[i], s[i], [img.shape[-1], img.shape[-2]], use_udp=False)
        
        all_preds = np.zeros((batch_size, output_joints.shape[1], 3), dtype=np.float32)
        all_boxes = np.zeros((batch_size, 6), dtype=np.float32)
        all_preds[:, :, 0:2] = output_joints
